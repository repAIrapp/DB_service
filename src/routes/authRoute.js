...
// --- règles de mot de passe (inchangées) ---
const passwordRules = [
  body('password')
    .if(body('authType').equals('local'))
    .isString().withMessage('Mot de passe invalide')
...
// 🔥 bloc FRONT_BASE_URL commun
const isProd = process.env.NODE_ENV === 'production';
const FRONT_BASE_URL = (() => {
  const v = (process.env.FRONTEND_URL || '').trim().replace(/\/+$/, '');
  if (isProd && !v) {
    throw new Error('FRONTEND_URL manquante en production');
  }
  ...
  async (req, res) => {
    try {
      ...
      if (authType === 'local') {
        ...
        const confirmationLink = `${FRONT_BASE_URL}/verify?userId=${newUser._id}`;
        console.log('[MAIL][DB] FRONTEND_URL used =', FRONT_BASE_URL);
        console.log('[MAIL][DB] confirmationLink =', confirmationLink);

        try {
          await axios.post(`${process.env.NOTIF_BASE_URL}/api/email/confirmation`, {
            email: newUser.email,
            confirmationLink,
          });
        } catch (err) {
