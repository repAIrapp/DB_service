...
// //         console.error("Erreur envoi e-mail confirmation :", err.message);
// //         console.error("Détails retour notif-service :", err.response?.status, err.response?.data);
// //       }
...
//       return true;
//     }),
//   // Confirmation OBLIGATOIRE si authType=local
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (authType === 'local') {
        // ⛔️ PAS DE hash ici : ton modèle Mongoose fait déjà le hash en pre('save')
        const newUser = await createUser({
          email,
